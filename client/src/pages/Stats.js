...
//chart js required in scope for component
...
const Stats = (props) => {
  ...
  //hits the backend endpoint to retrieve all of the book data from the database.
  useEffect(() => {
    axios
      .get("/api/v1/books", {
        headers: {
  ...
  //get an array with all the years when the books were read.
  const yearsWhenRead = books.map((book) => {
    return books.length > 0 ? book.date.split("-")[0] : "";
  });

  //create an array with just the unique years. Sort the array with the most recent year coming first. This will be used as the labels for the chart
  ...
  for (let i = 0; i < yearsWhenRead.length; i++) {
    ...
    if (!countsObject.hasOwnProperty(yearsWhenRead[i])) {
      ...
      //if the year is already in the object, add one to the book count.
  ...
  //reverse the counts to match the labels on the chart. This ensures the most recent year will be on the far left of chart.
  counts.reverse();
